...
if (game === 'yes') {
    ...
    alert('Too bad we\'re playing.')
}

// FIRST QUESTION

function questionOne(score) {
...
function questionSeven(score) {
    ...
    while (tries <= 6) {
        ...
        if (!ind) { // if indicator is false run alert and add a try
            ...
            tries += 7
            alert('Correct, one of my favorite cities is ' + q7 + '. My favorite cities are MIAMI, SEATTLE, DALLAS, HOUSTON, OAKLAND, and BROOKLYN.')
        }
        if (tries === 6) {
            alert('Sorry ' + name + ' you ran out of tries.')
            break;
        }
...
if (score >= 5) {
    alert('So, ' + name + ' you got ' + score + ' out of 7 on my questions. Great Job!') // opens last window stating this.
}
else {
    alert('So, ' + name + ' you only got ' + score + ' out of 7 on my questions. Try harder next time.') // opens last window stating this.
}